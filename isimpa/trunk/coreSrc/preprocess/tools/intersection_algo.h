...
/* if USE_APPROXIMATION_TEST is true then we do a check: 
         if |dv|<APPROXIMATION then dv=0.0;
   else no check is done (which is less robust)
*/
#define USE_APPROXIMATION_TEST TRUE  
#ifndef APPROXIMATION
	//#define APPROXIMATION 0.001
	#define APPROXIMATION 0.005	
#endif
...
#if USE_APPROXIMATION_TEST==TRUE
  if(fabs(du0)<APPROXIMATION) du0=0.0;
  if(fabs(du1)<APPROXIMATION) du1=0.0;
  if(fabs(du2)<APPROXIMATION) du2=0.0;
#endif
  du0du1=du0*du1;
  du0du2=du0*du2;
...
#if USE_APPROXIMATION_TEST==TRUE
  if(fabs(dv0)<APPROXIMATION) dv0=0.0;
  if(fabs(dv1)<APPROXIMATION) dv1=0.0;
  if(fabs(dv2)<APPROXIMATION) dv2=0.0;
#endif
