	...
	*/
	...
	void SG_Area::Draw(wxDC& thisDc,const wxSize& drawSize)
	{
		...
		for( it = m_owner->elements.begin(); it != m_owner->elements.end(); ++it )
		{
			...
			//wxLogInfo(_("Temps total de l'opération de dessin %i ms"),durationOperation.GetValue());
		#endif
	...
	SG_Legend::SG_Legend(simpleGraph* parent,LEGEND_PLACEMENT _whereToInsert ,wxWindowID id,const wxSizerFlags& sizerParams ,const wxPoint& pos,
		...
		whereToInsert(_whereToInsert)
	{
		...
		wxSizer* sizerToAttach=parent->GetSizer(whereToInsert);

		if((whereToInsert==LEGEND_PLACEMENT_HORIZONTAL && sizerParams.GetFlags()==wxALIGN_LEFT) || (whereToInsert==LEGEND_PLACEMENT_VERTICAL && sizerParams.GetFlags()==wxALIGN_TOP))
			sizerToAttach->Prepend(this,sizerParams);
		else
			sizerToAttach->Add(this,sizerParams);
