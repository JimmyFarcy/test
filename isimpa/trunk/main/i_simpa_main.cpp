...
{
	wxLaunchDefaultApplication("doc/documentation/manuel_I_Simpa.pdf");
}
void MainUiFrame::OnFileSppsDoc(wxCommandEvent& event)
{
	wxLaunchDefaultApplication("doc/documentation/SPPS_manuel.pdf");
}

void MainUiFrame::OnOpenFile (wxCommandEvent & event)
{
	smart_ptr<wxFileDialog> openFileDialog = new wxFileDialog ( this, _("Importer une scène"), "", "", _("Tous les formats compatibles|*.3ds;*.ply;*.bin;*.poly;*.stl|Fichier 3DS (*.3ds)|*.3ds|Fichier poly(*.poly)|*.bin|Fichier ply (*.ply)|*.ply|Fichier binaire 1.00(*.bin)|*.bin|Fichier stéréolithographie(*.stl)|*.stl"),
